        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {services.map((service, index) => (
            <Link
              key={service.name}
              href="/services"
              className={`group flex items-center justify-between p-5 rounded-xl bg-background border border-border/30 hover:border-primary/20 hover:shadow-md boty-transition hover:-translate-y-0.5 scroll-reveal-scale ${isVisible ? "visible" : ""} stagger-${index + 1}`}
            >
              <div className="flex items-center gap-3">
                <h3 className="text-sm font-medium text-foreground group-hover:text-primary boty-transition">
                  {service.name}
                </h3>
                {service.tag && (
